...
import type { UserProfile, ProfessorProfile, AnalysisResult, AppView, SavedProfessor, TieredUniversities, ProfessorRecommendation, Sop, ProgramDiscoveryResult, ProgramDetails, SavedProgram, UniversityWithPrograms } from './types';
...
// A mock auth function for demonstration. In a real app, this would be a call to a backend.
const mockAuthenticate = (email: string, pass: string) => {
    // For this demo, any non-empty email/password is valid.
    if (email && pass) {
...
function App() {
    ...
    // App State - Multi-profile support
    ...
    const [isRegenerating, setIsRegenerating] = useState(false); // For modal regeneration
    const [error, setError] = useState<string | null>(null);
    const [analyzingSavedProfId, setAnalyzingSavedProfId] = useState<string | null>(null); // For saved professor analysis/regeneration

    // Discovery Tab State (most loading/error states moved to global)
    ...
    // Global Loading States for background tasks
    ...
    useEffect(() => {
        // One-time migration from old single-profile structure to multi-profile
        ...
        if (oldProfileRaw && profiles.length === 0) {
            try {
                ...
                const newProfile: UserProfile = {
                    ...
                    ...oldProfile, // Spread old data over the new structure
                    // Handle transition to new degree structure
                    bachelor: {
            ...
            } catch (e) { console.error("Failed to migrate old profile.", e); }
    ...
    const isProfileComplete = useMemo(() => {
        ...
        return !!(activeProfile.name && activeProfile.academicSummary && activeProfile.researchInterests && activeProfile.bachelor?.major);
    }, [activeProfile]);
    ...
    const handleRegenerateForModal = async (prompt: string) => {
        ...
        try {
            ...
            setAnalysisModalState(prev => ({
                ...prev,
                result: { ...prev.result!, ...regeneratedData }
            }));
        } catch (e: any) {
    ...
    const handleGenerateEmailForDiscoveredProf = async (prof: ProfessorRecommendation, profileToUse: UserProfile, papers: string[]) => {
        ...
        const profProfile: ProfessorProfile = {
            name: prof.name,
            university: prof.university,
            department: prof.department,
            email: prof.email,
            labWebsite: prof.labWebsite,
            researchFocus: prof.researchSummary,
        };
    ...
    const handleSaveProfessor = (prof: ProfessorRecommendation) => {
        const newSavedProf: SavedProfessor = {
            ...
            emailSent: false,
            outcome: 'pending',
            suggestedPapers: prof.suggestedPapers || [],
        };
    ...
    const handleUpdateSavedProfessor = (updatedProf: SavedProfessor) => {
        setSavedProfessors(prev => prev.map(p => p.id === updatedProf.id ? updatedProf : p));
    };

    const handleDeleteSavedProfessor = (profToDelete: SavedProfessor) => {
        setSavedProfessors(prev => prev.filter(p => p.id !== profToDelete.id));
    };
    
    const handleAddSavedProfessor = (newProf: SavedProfessor) => {
        setSavedProfessors(prev => [...prev, newProf]);
    };
    ...
    const handleAnalyzeSavedProfessor = async (prof: SavedProfessor, papers: string[]) => {
        ...
        try {
            ...
            setSavedProfessors(prev => prev.map(p => 
                p.id === prof.id ? { ...p, ...result } : p
            ));
        } catch (e: any) {
            setError(e.message || `Failed to analyze ${prof.name}.`);
        } finally {
            setAnalyzingSavedProfId(null);
        }
    };
    ...
    const handleRegenerateForSavedProfessor = async (prof: SavedProfessor, prompt: string) => {
        ...
        const previousResult: AnalysisResult = {
            alignmentSummary: prof.alignmentSummary,
            outreachEmail: prof.outreachEmail,
        ...
        try {
            ...
            setSavedProfessors(prev => prev.map(p => 
                p.id === prof.id ? { ...p, ...regeneratedData } : p
            ));
        } catch (e: any) {
            setError(e.message || `Failed to regenerate email for ${prof.name}.`);
        } finally {
            setAnalyzingSavedProfId(null);
        }
    };
    ...
    const handleManualGenerateAndSaveEmail = async (profData: ProfessorProfile, papers: string[], profileToUse: UserProfile) => {
        if (!profileToUse) {
            setError("Please select a profile to use for generation.");
            return;
        }
        setIsDiscoveryLoading(true);
        setError(null);
        try {
            ...
            const newSavedProfessor: SavedProfessor = {
                id: `${profData.name}-${profData.university}`.replace(/\s+/g, '-'),
                name: profData.name,
                university: profData.university,
                department: profData.department,
                email: profData.email,
                labWebsite: profData.labWebsite,
                researchSummary: profData.researchFocus,
                feedback: '',
                emailSent: false,
                outcome: 'pending',
                ...result
            ...
            setActiveView('generate'); // Navigate to generate tab to see the result
        } catch (e) {
    ...
    const handleFindProgramsForSpecificUniversity = async (universityName: string, profileToUse: UserProfile, keywords: string) => {
        ...
        try {
            ...
            setProgramDiscoveryResults(results);
             ...
             setError(e.message || "Failed to find programs for this university. Please try again.");
        } finally {
            setIsProgramDiscoveryLoading(false);
        }
    }
    ...
    const handleFindProgramsBroadly = async (country: string, profileToUse: UserProfile, keywords: string) => {
        ...
        try {
            ...
            setProgramDiscoveryResults(results);
             ...
             setError(e.message || "Failed to find programs. Please try a different query.");
        } finally {
            setIsProgramDiscoveryLoading(false);
        }
    }
    ...
    const handleLoadMoreProgramsBroadly = async (country: string, profileToUse: UserProfile, keywords: string) => {
        if (!programDiscoveryResults) return; // Can't load more if there's nothing to add to
        ...
        try {
            ...
            setProgramDiscoveryResults(prev => ({
                ...prev!,
                universities: [...(prev?.universities || []), ...newResults.universities]
            }));
        } catch (e: any) {
             setError(e.message || "Failed to load more programs.");
        } finally {
            setIsProgramDiscoveryLoadMoreLoading(false);
        }
    }
    ...
    const handleOpenSopModal = (professor: ProfessorProfile | ProfessorRecommendation | SavedProfessor, university: string, papers: string[] = [], profileId?: string) => {
        setSopGenerationState({ isOpen: true, professor, university, papers, profileId });
    };

    const handleCloseSopModal = () => {
        setSopGenerationState({ isOpen: false, professor: null, university: '', papers: [] });
    };
    ...
    const handleGenerateSopFromModal = async (program: string) => {
        ...
        try {
            ...
            const newSop: Sop = {
                ...
                program,
                content,
                targetProfessors: [{ name: professor.name, researchSummary: 'researchSummary' in professor ? professor.researchSummary : ('researchFocus' in professor ? professor.researchFocus : '') }],
            ...
            setError(e.message || "Failed to generate SOP.");
        } finally {
    ...
    const discoverViewProps = {
      professorDiscoveryProps: {
        ...
        savedProfessorIds: savedProfessorIds,
    ...
    const generateViewProps = {
        ...
        sopTabProps: {
            ...
            setSops: setSops,
            newlyCreatedSopId: newlyCreatedSopId,
